        ...
        <div class="mb-4" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div>
                @if($activeRegister)
                    ...
                    <button onclick="showAdjustmentModal('entry')" class="btn btn-success me-2"
                        style="border:none; padding:10px 20px; font-weight:600; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
                        ➕ Entrada
                    </button>
                    <button onclick="showAdjustmentModal('exit')" class="btn btn-warning me-2"
                        style="border:none; padding:10px 20px; font-weight:600; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1); color: #fff;">
                        ➖ Salida
                    </button>
                    <button onclick="showCloseBoxModal()" class="btn btn-danger me-2"
                        style="border:none; padding:10px 20px; font-weight:600; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
                        🔒 Cerrar Caja (Cuadre)
                    </button>
                @else
                    <span class="badge bg-danger me-2" style="font-size: 1rem; padding: 10px;">🔴 Caja Cerrada</span>
                    <button onclick="showOpenBoxModal()" class="btn btn-success me-2"
                        style="border:none; padding:10px 20px; font-weight:600; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
                        🔓 Apertura de Caja
                    </button>
                @endif
                <a href="{{ route('cash-registers.index') }}" class="btn btn-secondary me-2"
                    style="border:none; padding:10px 20px; font-weight:600; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
                    📜 Historial
                </a>
        ...
        <div
            ...
            <div class="card mb-4 border-0 shadow-sm" style="border-radius: 15px; overflow: hidden;">
                <div class="card-header border-0 shadow-sm"
                    style="background: linear-gradient(135deg, #8B0000 0%, #e53935 100%); padding: 25px;">
                    <h4 class="mb-0 fw-bold text-white" style="letter-spacing: 0.5px;" align="center">Resumen Financiero del
                        Día</h4>
                    <p class="mb-0" style="color: rgba(255,255,255,0.8); font-size: 0.85rem;">Visión global de movimientos y
                        utilidad</p>
                        ...
                        <div class="col-md-4 border-end">
                            <div class="p-4" style="background-color: #f8fff9; height: 100%;">
                                ...
                                <div class="mb-4">
                                    <div class="d-flex justify-content-between align-items-end mb-1">
                                        ...
                                        <span class="fs-5 fw-bold"
                                            style="color: #2e7d32; font-family: 'Roboto', sans-serif;">
                                ...
                                <div class="d-flex flex-column gap-2 mb-4">
                                    <div
                                        class="d-flex justify-content-between small text-muted border-bottom border-success border-opacity-25 pb-1">
                                        <span>💵 Pagos en Efectivo (Caja)</span>
                                        <span class="fw-bold text-dark">$
                                            {{ number_format($salesTodayCash, 0, ',', '.') }}</span>
                                    </div>
                                    <div
                                        class="d-flex justify-content-between small text-muted border-bottom border-success border-opacity-25 pb-1">
                                        <span>📱 Pagos por Nequi</span>
                                        <span class="fw-bold text-dark">$
                                            {{ number_format($salesTodayNequi, 0, ',', '.') }}</span>
                                    </div>
                                    <div
                                        class="d-flex justify-content-between small text-muted border-bottom border-success border-opacity-25 pb-1">
                                        <span>🏦 Pagos por Bancolombia</span>
                                        <span class="fw-bold text-dark">$
                                            {{ number_format($salesTodayBancolombia, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between small text-muted pb-1">
                                        <span>📝 Créditos Nuevos (Por Cobrar)</span>
                                        <span class="fw-bold text-warning">$
                                            {{ number_format($salesTodayCredit, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="p-3 rounded-3" style="background: rgba(46, 125, 50, 0.1);">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <span class="fw-bold text-success">$
                                            {{ number_format($collectedReceivablesToday, 0, ',', '.') }}</span>
                                    </div>
                                </div>

                                <!-- Estimated Profit -->
                                <div class="p-3 rounded-3 mt-2"
                                    style="background: rgba(255, 152, 0, 0.1); border-left: 4px solid #ff9800;">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <span class="fw-bold small" style="color: #e65100;">✨ Estado de resultados del
                                            dia</span>
                                        <span class="fw-bold" style="color: #e65100;">$
                                            {{ number_format($profitToday, 0, ',', '.') }}</span>
                                    </div>
                                    <small class="text-muted d-block mt-1" style="font-size: 0.7rem;">(Ventas - Costos de
                                        ventas - Gastos)</small>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="p-4" style="background-color: #fffafa; height: 100%;">
                                ...
                                <div class="mb-4">
                                    <div class="d-flex justify-content-between align-items-end mb-1">
                                        ...
                                        <span class="fs-5 fw-bold"
                                            style="color: #c62828; font-family: 'Roboto', sans-serif;">
                                            - $
                                ...
                                <div class="d-flex flex-column gap-2">
                                    <div class="d-flex justify-content-between align-items-center p-2 rounded small"
                                        style="background: white; border: 1px solid #ffebee;">
                                        <span class="text-danger">🧾 Gastos (Efectivo)</span>
                                        <span class="fw-bold text-danger">- $
                                            {{ number_format($expensesTodayCash, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center p-2 rounded small"
                                        style="background: white; border: 1px solid #ffebee;">
                                        <span class="text-danger">🧾 Gastos (Bancolombia)</span>
                                        <span class="fw-bold text-danger">- $
                                            {{ number_format($expensesTodayBancolombia, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center p-2 rounded small"
                                        style="background: white; border: 1px solid #ffebee;">
                                        <span class="text-danger">🧾 Gastos (Nequi)</span>
                                        <span class="fw-bold text-danger">- $
                                            {{ number_format($expensesTodayNequi, 0, ',', '.') }}</span>
                                    </div>

                                    <!-- Costo de Ventas (Purchases) Breakdown -->
                                    <div class="p-2 rounded border border-danger border-opacity-25"
                                        style="background: #fff8f8;">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <span class="text-danger fw-bold small">🛒 Costo de Ventas (Compras)</span>
                                            <span class="fw-bold text-danger">- $
                                                {{ number_format($cashPurchases + $nequiPurchases + $bancolombiaPurchases + $creditPurchases, 0, ',', '.') }}</span>
                                        </div>
                                    ...
                                    <div class="p-2 rounded border border-danger border-opacity-25"
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            ...
                                            <span class="fw-bold text-danger">- $
                                                {{ number_format($paidPayablesToday, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between small text-muted ps-2">
                                            <span>Efectivo:</span>
                        ...
                        <div class="col-md-4">
                            <div class="p-4" style="background-color: #f0f7ff; height: 100%;">
                                ...
                                <div class="d-flex flex-column gap-3">
                                    <div
                                        class="d-flex justify-content-between border-bottom border-primary border-opacity-25 pb-2">
                                        <span class="text-muted small">Saldo Inicial (Base Apertura)</span>
                                        <span class="fw-bold text-secondary">$
                                            {{ number_format($previousDayBalance, 0, ',', '.') }}</span>
                                    </div>
                                    ...
                                    <div
                                        class="d-flex justify-content-between text-danger border-bottom border-primary border-opacity-25 pb-2">
                                        <span class="small">(-) Salidas (Gastos + Compras + Salidas)</span>
                                        <span class="fw-bold">- $
                                            {{ number_format($totalCashOutgoAll, 0, ',', '.') }}</span>
                                    </div>
                                    @if($adjExitCash > 0)
                                        <div
                                            class="d-flex justify-content-between text-danger border-bottom border-primary border-opacity-25 pb-2">
                                    ...
                                    <div class="alert alert-primary mb-0 mt-2 text-center shadow-sm"
                                        style="border: none; background: #e3f2fd; color: #0d47a1;">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm"
                    style="border-radius: 15px; background: linear-gradient(135deg, #fdf2f9 0%, #ffffff 100%);">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="rounded-circle bg-white shadow-sm p-3 me-3" style="font-size: 1.5rem;">📱</div>
                            <div>
                                <h6 class="text-uppercase fw-bold mb-0" style="color: #d81b60; letter-spacing: 1px;">Saldo
                                    Nequi</h6>
                                <small class="text-muted text-uppercase" style="font-size: 0.7rem;">Conciliación
                                    Digital</small>
                            </div>
                        </div>
                        <div class="row align-items-center">
                            <div class="col-7">
                                <div class="display-6 fw-bold mb-0" style="color: #333;"><span>$
                                        {{ number_format($nequiBalance, 0) }}</span></div>
                            </div>
                            <div class="col-5 text-end small text-muted">
                                <div class="mb-1">Base: <span class="fw-bold">$ {{ number_format($baseNequi, 0) }}</span>
                                </div>
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm"
                    style="border-radius: 15px; background: linear-gradient(135deg, #f0f7ff 0%, #ffffff 100%);">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="rounded-circle bg-white shadow-sm p-3 me-3" style="font-size: 1.5rem;">🏦</div>
                            <div>
                                <h6 class="text-uppercase fw-bold mb-0" style="color: #0d47a1; letter-spacing: 1px;">Saldo
                                    Bancolombia</h6>
                                <small class="text-muted text-uppercase" style="font-size: 0.7rem;">Conciliación
                                    Digital</small>
                            </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col-7">
                                <div class="display-6 fw-bold mb-0" style="color: #333;"><span>$
                                        {{ number_format($bancolombiaBalance, 0) }}</span></div>
                            </div>
                            <div class="col-5 text-end small text-muted">
            ...
            <div style="flex: 2; min-width: 350px; margin-bottom: 20px;">
                <div class="card h-100" style="border: none; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <div class="card-header bg-white"
                        style="border-bottom: 1px solid #f0f0f0; padding: 20px; border-radius: 15px 15px 0 0;">
